class CP_Child():
    ...
    def __init__(self, c_id:int, age:int, family_id: int, initial_daycare_id:int, actual_daycare_id: int, 
        ...
        self.pref = [d_id if d_id != None else 9999 for d_id in preference_list]    # replace None with 9999
        # additional attributes
        self.projected_pref = []
...
class CP_Daycare():
    ...
    def __init__(self, d_id:int, recruiting_numbers_list:list[int], share_ages_list:list[list[int]], 
                    priority_child_id_list: list[int], priority_score_list: list[float]):
        # attributes from dictionary
        self.id = d_id
                ...
                if c.age in self.all_shared_ages:         
                    for ages in self.share_ages_list:
                        if c.age in ages:      # if c belongs to some group`ages` in self.share_ages_list
                            for age in ages:  # add child to each priority_age_share_dic[age] with age in group`ages`
                                if c.id not in self.priority_age_share_dic[age]:   # caution : use `age` instead of `c.age`
    ...
    def return_better_children_than_child_excluding_siblings(self, child_id, children, allow_share_bool=True, exclude_bool=True):
        '''
        Input:
        ...
        '''
        better_children_id = []
        rank_dic = self.priority_age_share_dic if allow_share_bool == True else self.priority_age_dic
    ...
    def return_weak_better_children_than_child_excluding_siblings(self, child_id, children, allow_share_bool=True, exclude_bool=True, search_depth=5):
        '''
        Input:
            child_id: given child_id
            children: all children list
        ...
        for index in range(end):
            ...
            if index < pos:
                ...
                if exclude_bool==True:
                    if abs(c_score / child_score - 1) <= 0.00001 and c.id != child.id and c.family != child.family and c.id not in better_children_id: 
                        better_children_id.append(c.id)
                else:
                    if abs(c_score / child_score - 1) <= 0.00001 and c.id != child.id and c.id not in better_children_id: 
                        better_children_id.append(c.id)

        ############################################
        # #  test non-wasteful only
        # # very slow for large markets
        ############################################
#         child = next((c for c in children if c.id == child_id), None)
#         for index in range(len(rank_dic[child.age])):
#             c = next((c for c in children if c.id == rank_dic[child.age][index]), None)
#             if c.id != child.id and c.family != child.family and c.id not in better_children_id:
#                 better_children_id = [c_id for c_id in rank_dic[child.age] if c_id != child_id]
...
class CP_Family():
    ...
    def __init__(self, f_id:int, children_id_list:list=[int], pref_list:list=[int], assignment_daycare_id_list = None):
        self.id = f_id
    ...
    def return_siblings_for_certain_position_daycare_age(self, position, daycare_id, age, share_bool, children, daycares):
        '''
    ...
    def return_lowest_sibling_for_certain_position_daycare_age(self, position, daycare_id, age, share_bool, children_list, daycare_list):
        '''
        return the worst sibling who i) apply to daycare_id at given position and ii) belong to the same age or age group
        '''
        children_id_age = self.return_siblings_for_certain_position_daycare_age(position, daycare_id, age, share_bool, children_list, daycare_list)
        ...
        rank_dic = daycare.priority_age_share_dic if share_bool == True else daycare.priority_age_dic
