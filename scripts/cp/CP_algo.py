...
from scripts.cp.helper_functions import create_agents, get_agent

class InfeasibleError(Exception):
    pass

def CP(children_dic, daycares_dic, families_dic, share_bool, bp_num=0, solver_time=360, exclude_bool=True, search_depth=5):
    model = cp_model.CpModel()
    children, daycares, families = create_agents(children_dic, daycares_dic, families_dic)
    xfp, xcd, alpha, gamma_fp, gamma_fpd, gamma_fpdg, age_fpd, beta = create_variables(children, daycares, families, share_bool, model, exclude_bool, search_depth)
    ...
    all_xcd = [xcd[c.id, d_id] for c in children for d_id in c.all_daycare_ids if d_id != 9999]
    ...
    if status != cp_model.OPTIMAL:
        raise InfeasibleError('The problem is infeasible')
    ...
    return outcome_children_dic, outcome_fp, children, daycares, families
...
def creat_variables_gamma_siblings(children, daycares, share_bool, f, xcd, alpha, gamma_fp, gamma_fpd, gamma_fpdg, age_fpd, model, exclude_bool, search_depth):
    for p in range(len(f.pref)):    # for each position p, add one constraint for gamma[f, p]
        for d_id in f.return_daycare_id_for_certain_position(p): # D(f, p)
            ...
            capacity = d.total_numbers_share if share_bool == True else d.total_numbers
            for g in range(6):
                # calculate the number of children who i) are from family f ii) have grade related to g iii) apply to d at \succ_{f, p}
                number_fpdg = len(f.return_siblings_for_certain_position_daycare_age(p, d_id, g, share_bool, children, daycares))
                if number_fpdg != 0:
                    gamma_fpdg[f.id, p, d_id, g] = model.NewBoolVar(f'gamma_fpdg_[{f.id}, {p}, {d_id}, {g}]')
                    ...
                    worst_id = f.return_lowest_sibling_for_certain_position_daycare_age(p, d_id, g, share_bool, children, daycares)
                    # find all children who i) are not from family f ii) have higher priority than c*

                    children_better = d.return_weak_better_children_than_child_excluding_siblings(worst_id, children, share_bool, exclude_bool, search_depth)
                    ...
                    if len(children_better) == 0:  # when no child better than c* exists
                        ...
                        model.Add( (sum(xcd[c_id, d.id] for c_id in children_better) + number_fpdg) <= capacity[g]).OnlyEnforceIf(gamma_fpdg[f.id, p, d_id, g])
                        model.Add( (sum(xcd[c_id, d.id] for c_id in children_better) + number_fpdg) > capacity[g]).OnlyEnforceIf(gamma_fpdg[f.id, p, d_id, g].Not())
            # variable \gamma[f,p,d]
            gamma_fpd[f.id, p, d_id] = model.NewBoolVar(f'gamma_fpd_[{f.id}, {p}, {d_id}]')
            model.Add(sum(gamma_fpdg[f.id, p, d_id, g] for g in age_fpd[f.id, p, d_id]) == len(age_fpd[f.id, p, d_id])).OnlyEnforceIf(gamma_fpd[f.id, p, d_id])
            model.Add(sum(gamma_fpdg[f.id, p, d_id, g] for g in age_fpd[f.id, p, d_id]) < len(age_fpd[f.id, p, d_id])).OnlyEnforceIf(gamma_fpd[f.id, p, d_id].Not())
        # variable \gamma[f,p]
        gamma_fp[f.id, p] = model.NewBoolVar(f'gamma_fp_[{f.id}, {p}]')
        id_fp = f.return_daycare_id_for_certain_position(p)
        model.Add(sum(gamma_fpd[f.id, p, d_id] for d_id in id_fp) == len(id_fp)).OnlyEnforceIf(gamma_fp[f.id, p])
        model.Add(sum(gamma_fpd[f.id, p, d_id] for d_id in id_fp) < len(id_fp)).OnlyEnforceIf(gamma_fp[f.id, p].Not())

# creat gamma
def creat_variables_gamma(children, daycares, families, share_bool, xcd, alpha, model, exclude_bool, search_depth):
    gamma_fpdg = {}
    gamma_fpd = {}
    gamma_fp = {}
    age_fpd={}
    ...
    for f in families:
        creat_variables_gamma_siblings(children, daycares, share_bool, f, xcd, alpha, gamma_fp, gamma_fpd, gamma_fpdg, age_fpd, model, exclude_bool, search_depth)
...
def create_variables(children, daycares, families, share_bool, model, exclude_bool, search_depth):
    ...
    gamma_fp, gamma_fpd, gamma_fpdg, age_fpd = creat_variables_gamma(children, daycares, families, share_bool, xcd, alpha, model, exclude_bool, search_depth)
...
def feasibility_constraints(children, daycares, families, share_bool, xfp, xcd, model):
    ...
    for d in daycares:
        rank_dic = d.priority_age_share_dic if share_bool == True else d.priority_age_dic
        for g in range(6):
            if len(rank_dic[g]) != 0:
                if share_bool == False:
